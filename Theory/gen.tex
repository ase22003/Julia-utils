...
For each \(l\in\text{dom}(\Phi_n)\), then, a set of vectors \(\solarr\in\mathscr{G}^{<\omega}\) can be inferred with \(\kappa:\Phi\times G\to\mathscr{G}^{<\omega}\)\color{red}!!!!!!!!! + should \(\mathscr{G}\) be included too?\color{black}, defined by 
\[
	\kappa(\Phi, g) = \condw{
		\conde{\set{g}}{g\in\mathscr{G}}
		%\conde{\underset{k\in\kappa\prnth{\Phi, \Phi\prnth{o}}}\bigcup k}{\text{otherwise}}
		\conde{\underset{k\in\Phi(g)}\bigcup {\prnth{\underset{j\in\kappa(k)}{\scalebox{2}{$\vcat$}}}} }{\text{otherwise}}
	}
\]
