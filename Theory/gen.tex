...
Let \(G:=\left( \left\langle \mathscr{G} \right\rangle, * \right)\). Let \(\triangle:\vect{\Phi, L^2} \to \Phi\), where \(\Phi\) is the inverse map form of a pattern \(\mathfrak{P} \subseteq G^{<\omega}\) and \(L = \text{Dom}(\Phi) \subseteq G\), be defined by \(\forall l\in\text{Dom}(\Phi_1)\ \left(\Phi_1(l) \subseteq \Phi_2(l)\right)\) and \(\vect{l_1,l_2} \in \Phi_2(l_1*l_2)\) where \(\Phi_2 = \triangle(\Phi_1, \vect{l_1, l_2})\) and \(l_1, l_2 \in \text{Dom}(\Phi_1)\).
