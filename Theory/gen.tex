...
%\textit{concepts}.
The \textit{inverse map form} of a pattern \(\mathfrak{P}\) is a function \(\Phi:L \to \mathcal{P}(\mathfrak{P})\), where \(\Phi(l) = \setdef{p\in\mathfrak{P}}{\Pi(p)=l}\) for \(l\in L\).

Let \(G:=\left( \left\langle \mathscr{G} \right\rangle, * \right)\) be a group. Let \(\triangle:\Phiset \times L^2 \to \Phiset\) -- where \(\Phiset\) is the set of all inverse map forms of a pattern \(\mathfrak{P} \subseteq G^{<\omega}\) and \(L \subseteq G\) -- be defined by 
%\(\forall l\in\text{dom}(\Phi_1)\setminus\set{l_1*l_2}\ \left(\Phi_2(l) = \Phi_1(l)\right)\) and \(\Phi_2(l_1*l_2) = \Phi_1(l_1*l_2)\cup\set{\vect{l_1,l_2}}\) if \(l_1*l_2\in\text{dom}(\Phi_1)\) and \(\Phi_2(l_1*l_2) = \set{\vect{l_1,l_2}}\) if not -- where \(\Phi_2 = \triangle(\Phi_1, \vect{l_1, l_2})\) and \(l_1, l_2 \in \text{dom}(\Phi_1)\).
\(\forall l\in\text{dom}(\Phi)\setminus\set{l_1*l_2}\ \left(\Phi'(l) = \Phi(l)\right)\) and \(\Phi'(l_1*l_2) = \Phi(l_1*l_2)\cup\set{\vect{l_1,l_2}}\) if \(l_1*l_2\in\text{dom}(\Phi)\) and \(\Phi'(l_1*l_2) = \set{\vect{l_1,l_2}}\) if not -- where \(\Phi' = \triangle(\Phi, \vect{l_1, l_2})\) and \(l_1, l_2 \in \text{dom}(\Phi)\).
...
Consider the function \(\mathscr{R}:\Phiset \times \mathbb{N} \to \Phiset\) defined by 
\[
...
\]
Now, let \(\Phi_n = \mathscr{R}(\Phi, n)\), where 
%\(\text{dom}(\Phi) = \mathscr{G}\) and \(\text{Img}(\Phi) = \set{\emptyset}\) 
\(\Phi:\mathscr{G} \to \set{\emptyset}\) 
and \(n\in\mathbb{N}\). Then \(\Phi_n\) is a partial or full representation of the group \(G\), where each element in \(\text{dom}(\Phi_n)\) maps to the set of ordered pairs of elements in \(G\) to which it is mapped by \(*\).

For each \(l\in\text{dom}(\Phi_n)\), then, a set of vectors \(\solarr\in\mathscr{G}^{<\omega}\) can be inferred with \(\kappa:\Phiset\times G\to\mathscr{G}^{<\omega}\)\color{red}!!!!!!!!! should \(\mathscr{G}\) be included in domain too? probably, yes!!!!!!!!\color{black}, defined by 
\[
	\kappa(\Phi, g) = \condw{
